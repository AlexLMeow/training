...
public class SwapChargeShoot {
    ...
    static long solve(boolean[] charging, long shield) {
        ...
        if (startDmg <= shield) { return minHacks; }
        if (nShoots(charging) > shield) { return -1; }
...
//        return -1;
    ...
    static int[] getNumShotsAfter(boolean[] chargeSeq) {
        ...
        for (int i = N-2; i >= 0; i--) {
            ...
            if (!chargeSeq[i+1]) { nShotsAfter[i]++; } // if next instruction was SHOOT, update cumulative count
        }
        return nShotsAfter;
    }
    ...
    static int nShoots(boolean[] chargeSeq) {
        int n = 0;
        for (boolean c : chargeSeq) { if (!c) n++; }
        return n;
    }
