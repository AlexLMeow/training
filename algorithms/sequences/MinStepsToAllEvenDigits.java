...
public class MinStepsToAllEvenDigits {
    ...
    static long minSteps(long N) {
        ...
        if (highestOddPower < 0) { return 0; } // already all even
        // find target number
    ...
    static boolean shouldGoUp(int highestOddPower, byte[] digits) {
        if (highestOddPower == 0) { return true; } // doesn't matter which direction
        if (digits[highestOddPower] == 9) { return false; } // cannot go up; will create new 1 in higher power
        for (int i = highestOddPower - 1; i >= 0; i--) {
            // 4 is the split point (54 -> 60 = 6 steps) (54 -> 48 = 6 steps)
            if (digits[i] > 4) { return true; }
            if (digits[i] < 4) { return false; }
        }
